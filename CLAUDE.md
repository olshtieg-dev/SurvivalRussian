...
Content-word buildout is mid-stream. Coverage now: **adverbs 98 ✅, verbs 104 ✅, adjectives 78 (39 pairs) ✅ binary-target met, nouns 40.** Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
2. **Nouns — 40 → 300+** (largest remaining; the main outstanding goal). Extend `nouns/` with new semantic buckets (food, nature, time, people/roles, clothing, work/money, transport…). Pull most-common nouns from `russian-frequency-1-1000.json`; propose bucket list to owner first.

(Adjectives Batch 1 = `ba48d93`; Batch 2 = the 14 pairs below, see work log.)
...
- Committed/pushed through **`35324f1`** (adverbs `9859f59`, verbs `3a8d71c`, adjectives Batch 1 `ba48d93`, doc `35324f1`). **Adjectives Batch 2 (14 pairs → 78 adjectives) is done but UNCOMMITTED** — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
