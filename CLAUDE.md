...
On branch `main`, up to date with `origin/main` at commit `b61cb12` ("infut fix" — the keyboard pure-positional change). Only uncommitted change is this session's CLAUDE.md doc edit. Start by re-orienting: read this file top-to-bottom, then `git status` to confirm.
...
**Repo state (as of 2026-06-05 commit `b61cb12`):**
- Branch: `main`, up to date with `origin/main`. (Other branches `stash`/`staging`/`testing` still exist; `main` is now the active working branch.)
- Working tree: only `CLAUDE.md` modified (this session's doc update); not yet committed.
