...
**Content-word buildout COMPLETE + extended (2026-06-05/06): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 667 (375 core + 292 from the 1001–2000 tier via N5+N6+N7).** All four families exceed target. `vocabulary.json` = 10,986 entries, 0 missing tokens corpus-wide, 0 Cyrillic-in-natural. What remains is OPTIONAL: ~139 of the 1001–2000 noun candidates still uncovered (a few of those are perennial non-nouns) — see below. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
- **Deeper noun coverage** — IN PROGRESS: N5+N6+N7 (2026-06-06) added **292 nouns** from the 1001–2000 tier → nouns **667** (committed `bb38339`). **N8 is PARTIAL & UNCOMMITTED (subagent session limit hit mid-batch, resets 5:10pm ET 2026-06-06):** ~**88 of 139** N8 lesson `.json` files were written into the existing `nouns/` subfolders but are UNTRACKED and NOT yet imported by any index.js — so the committed build is still valid (0 missing among imported lessons). NOTHING was regenerated or committed, deliberately.
  **TO RESUME N8 after 5:10pm reset:** (1) recompute still-uncovered = ё-folded-exclude current `nouns/` labels (which now include the 88 partial files, so they'll be treated as done) from `/tmp/freq-1001-2000.txt` — regenerate that freq file from the Wiktionary 1001–2000 page if `/tmp` was cleared; (2) classify+generate ONLY the remaining ~51 (drop perennial non-nouns деревянный/бедный/Европа/познакомиться); (3) regenerate ALL noun folder indexes (import existing+partial+new, metadata preserved); (4) `grep -P '[^\x00-\x7F]'` for non-ASCII filenames + check no stray `lessons/<bucket>/` top-level dirs; (5) missing-token scan → parallel vocab fill → validate 0 missing → commit N8. Finishing N8 ≈ nouns ~800. Beyond rank 2000 needs a deeper frequency source.
...
- **~300+ most common nouns** — **DONE: 667** (19 buckets; N1–N4 = top-1000 tier (375); N5+N6+N7 = +292 from the 1001–2000 tier, 2026-06-06; ~139 of that tier still uncovered).
...
- Committed/pushed through N6 `89ea288` (`.bak` backups are gitignored via `*.bak`/`*.bak.*`); **nouns Batch N7 about to be committed this session** (96 new nouns from the 1001–2000 tier, `vocabulary.json` at **10,986 entries**, 667 noun cards, 0 missing corpus-wide). Run `git log --oneline -3` for the latest hash. — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
