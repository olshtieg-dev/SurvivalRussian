...
Content-word buildout is nearly complete. Coverage now: **adverbs 98 ✅, verbs 104 ✅, adjectives 78 ✅, nouns 316 ✅ (300+ target met) — only the 3 merge-buckets (N4) remain to absorb the last ~59 nouns.** Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
2. **Nouns — 40 → 300+ — TARGET MET (316 nouns); only N4 (3 merges) left.** Full plan was 336 new nouns / 18 buckets, manifest `/tmp/nouns-manifest.json` (ephemeral — regen from `russian-frequency-1-1000.json` if gone). **N1 `0945019`:** family, food-drink, nature-weather, animals, time-units, transport, work-money (88). **N2 `549cac3`:** people-roles, communication-media, education-science (76). **N3 (committed this session):** society-state (26), emotions-states (20), abstract-mind (15), abstract-structure (23), abstract-event (28) = 112. nouns/ now has 19 group folders, 316 cards.
   **REMAINING — N4 = the 3 MERGE-into-existing buckets (~59 nouns):** body (~21 → merge into `body-person/`), places (~27 → merge into `motion-location/`), home-objects (~11 → merge into `household/`). For each: write the new `.json` files INTO the existing folder, then regenerate that folder's `index.js` to import existing+new while PRESERVING its export names (bodyPersonNounsLessonSets/Folder, motionLocationNounsLessonSets/Folder, householdNounsLessonSets/Folder) — do NOT rename exports or nouns/index.js needs no change. Then missing-token scan → parallel vocab fill → validate 0 missing → commit. Finishing N4 lands nouns ~375 and COMPLETES the whole content-word buildout (adverbs/verbs/adjectives/nouns all ✅).
...
- **~300+ most common nouns** — **DONE: 316** (target met; 19 buckets = original 4 + N1's 7 + N2's 3 + N3's 5, added 2026-06-05). Only N4 — 3 merge-buckets (~59 nouns into body-person/motion-location/household) — remains to reach ~375. See NEXT STEPS #2.
...
- Committed/pushed through nouns Batch N2 `549cac3`; **nouns Batch N3 about to be committed this session** (112 new noun lessons in 5 new `nouns/` subfolders, `vocabulary.json` at **8009 entries**, 0 missing corpus-wide, 316 noun cards — 300+ target met). Run `git log --oneline -3` for the latest hash. `.bak` backups remain intentionally untracked (now incl. `pre-noun3-fill`). — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
