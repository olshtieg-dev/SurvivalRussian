...
- **Adjectives Batch 3** — ~12 standalone adjectives with no antonym (главный, русский, общий, известный, целый, великий, нужный, готовый, возможный, необходимый + colors). Would need a standalone `adjectives/` family or a `binary-adjectives` sibling group. Binary target already met at 78, so optional.
...
- Committed/pushed through nouns Batch N3 `c5ac5a6`; **nouns Batch N4 about to be committed this session** (59 new noun lessons merged into body-person/motion-location/household, `vocabulary.json` at **8465 entries**, 0 missing corpus-wide, 375 noun cards). This completes the content-word buildout. Run `git log --oneline -3` for the latest hash. `.bak` backups remain intentionally untracked (8 of them now, incl. `pre-noun4-fill`); a `*.bak` line in `.gitignore` would hide them all. — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
