...
**Content-word buildout COMPLETE + extended (2026-06-05/06): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 917 (375 core + 431 from 1001–2000 tier + 111 from 2001–3000 tier).** All families far exceed target; top-2000 fully mined, into the 2001–3000 tier now. `vocabulary.json` = 13,002 entries, 0 missing tokens corpus-wide, 0 Cyrillic-in-natural. Nothing required remains — see below. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
2. **Nouns — DONE (375 nouns).** All 18 manifest buckets built across N1 `0945019`, N2 `549cac3`, N3 `c5ac5a6`, N4 (this session): the 3 merge-buckets (body→body-person, places→motion-location, home-objects→household) were folded into the existing folders (indexes regenerated preserving export names; уголь 'coal' lives as `motion-location/ugol2.json` to avoid the угол collision). nouns/ has 19 group folders, 375 cards.
...
- **Deeper noun coverage** — 1001–2000 tier done (N5–N8). **N9 (2026-06-06) started the 2001–3000 tier** (WebFetched from Wiktionary → `/tmp/freq-2001-3000.txt`, hand-pruned to ~763 noun-likely, but it still contains many verbs/adjectives so the classifier drops ~50% — keep that in mind, pull a generous slice per batch). N9 added 111 nouns → nouns **917**; **~648 of the 2001–3000 candidates remain** for N10+. Recipe unchanged: ё-folded-exclude current labels → classify (drops non-nouns) → generate → regen indexes → fill → commit. Beyond rank 3000 needs yet another WebFetch.
...
- **~100 most common adjectives** — **DONE: 95** (39 binary pairs = 78, + a standalone `adjectives/` family of 17: descriptive 10 + colors 7; all 2026-06-05; all cards 10 missions). **Owner preference (2026-06-05): favor BINARY antonym pairs wherever applicable** — descriptive adjectives mostly come in opposites, and pairing reinforces the nature of description. Keep the `binary-adjectives/` pair format; fall back to standalone only when a word has no natural antonym. **Optional remaining: ~12 standalone adjectives** (главный, русский, colors, etc. — no clean antonym) if owner wants ~90; binary target is already met.
- **~100 most common adverbs** — **DONE: 98** (built 2026-06-05; `adverbs/` family, 6 semantic buckets),
- **~300+ most common nouns** — **DONE: 917** (19 buckets; N1–N4 = top-1000 (375); N5–N8 = +431 from 1001–2000; N9 = +111 from 2001–3000, 2026-06-06). ~648 of the 2001–3000 tier remain for N10+.
...
- Committed/pushed through N8 `de9c9a4`; **nouns Batch N9 about to be committed this session** (111 new nouns from the 2001–3000 tier, `vocabulary.json` at **13,002 entries**, 917 noun cards, 0 missing corpus-wide). Run `git log --oneline -3` for the latest hash. (`.bak` backups gitignored.) — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
...
- **Nouns Batch N9: +111 nouns — started 2001–3000 tier (806 → 917)** (2026-06-06). WebFetched the Wiktionary 2001–3000 list; the hand-pruned candidate list still had many non-nouns so two classifier passes (top 100 → 48 nouns, next 140 → 63 nouns) yielded 111. Generated into existing 18 folders, regenerated indexes (917 files, 0 problems). Vocab fill: 877 tokens, 12 subagents → vocab **12,125 → 13,002**, 0 collisions, 0 stragglers, 0 missing corpus-wide. ~648 of the 2001–3000 candidates remain. Backup `vocabulary.json.bak.pre-noun9-fill`.
- **Nouns Batch N8: +139 nouns — 1001–2000 tier finished (667 → 806)** (2026-06-06, committed `de9c9a4`). Split across the 5:10pm session-limit reset: 88 lesson files written pre-limit (left untracked/unreferenced so the committed build stayed valid), then 51 generated after reset; indexes regenerated once at the end (806 files, 0 import problems). Vocab fill: 1139 tokens, 12 subagents → vocab **10,986 → 12,125**, 0 collisions, 0 stragglers, 0 missing corpus-wide. The 1001–2000 tier is now exhausted (only non-noun residue: деревянный/бедный/Европа/познакомиться). Backup `vocabulary.json.bak.pre-noun8-fill`.
- **Nouns Batch N7: +96 nouns from the 1001–2000 tier (571 → 667)** (2026-06-06, committed `bb38339`). Same recipe as N5/N6 (ё-folded exclusion, classify next 100, 4 dropped as adjective/proper-noun/verb, 10-mission decks, regenerate indexes). Vocab fill: 789 tokens, 10 subagents → vocab **10,197 → 10,986**, 0 collisions, 0 stragglers, 0 missing corpus-wide. ~139 of the 1001–2000 candidates remain. Backup `vocabulary.json.bak.pre-noun7-fill`.
- **Nouns Batch N6: +98 nouns from the 1001–2000 tier (474 → 571)** (2026-06-06, committed `89ea288`). Recomputed still-uncovered nouns with ё-FOLDED exclusion (fixes тетя/тётя false-new), classified the next 100 (2 adjectives dropped) into the existing 18 folders, generated 10-mission decks, regenerated all noun indexes. Removed one proper-noun slip (Европа — a subagent wrote `Еvropa.json` with a Cyrillic Е in the filename; deleted, since proper nouns are out of scope). Vocab fill: 773 tokens, 10 subagents → vocab **9424 → 10,197**, 0 collisions, 0 stragglers, 0 missing corpus-wide. ~138 of the 1001–2000 tier still uncovered. Backup `vocabulary.json.bak.pre-noun6-fill`. LESSON: subagents can emit proper nouns / non-ASCII filenames — always grep `find nouns -name '*.json' | grep -P '[^\x00-\x7F]'` and drop proper nouns before regenerating indexes.
- **Nouns Batch N5 + vocab gloss cleanup** (2026-06-06, committed `f2a1c0b`/`76e8425`). (a) Gloss cleanup: fixed 6 wrong glosses (рот→mouth, комплекс→complex, лист→leaf/sheet, тихо→quietly, сотрудник→employee, мочь→to-be-able-to) and stripped Cyrillic from 44 leaked `natural` fields; corpus now 0 Cyrillic-in-natural (commit `76e8425`). (b) N5 deeper nouns: WebFetched the Wiktionary 1001–2000 frequency tier, hand-pruned to noun candidates, excluded the 372 already-covered, classified the top 99 new nouns into the existing 18 folders via subagent, generated 10-mission decks, regenerated all 19 noun folder indexes (preserving metadata by extraction). Nouns 375→474. Vocab fill: 825 tokens, 10 subagents → vocab **8599→9424**, 0 collisions, 0 missing corpus-wide, 0 stragglers. ~335 more 1001–2000-tier noun candidates queued at `/tmp/freq-new-nouns.txt` (lines 101+). Backups `vocabulary.json.bak.pre-glossfix` and `.pre-noun5-fill`.
- **Standalone adjectives family: +17 (adjectives 78 → 95)** (2026-06-05, committed `be1472e`). New `adjectives/` family for words with no antonym (separate from `binary-adjectives/`): 2 groups — descriptive (главный, русский, общий, известный, целый, великий, нужный, готовый, возможный, необходимый) and colors (красный, зелёный, синий, жёлтый, серый, голубой, золотой). 10-mission agreement decks (m/f/n/pl across cases). Wired into `lessons/index.js` (import + lessonSets spread + lessonFolders with adjectivesFolder/adjectivesGroupFolders), mirroring the adverbs family. Vocab fill: 134 tokens, 3 subagents → vocab **8465 → 8599**, 0 collisions, 0 missing corpus-wide. Backup `vocabulary.json.bak.pre-adjstd-fill`.
- **Nouns Batch N4: +59 nouns / 3 merge-buckets (316 → 375 nouns) — content-word buildout COMPLETE** (2026-06-05, committed `e60e767`). Merged body (→body-person), places (→motion-location), home-objects (→household) into the existing folders by writing new `.json` into them and regenerating those 3 `index.js` to import existing+new while preserving export names (nouns/index.js untouched). уголь 'coal' filed as `motion-location/ugol2.json` to avoid the угол collision. Vocab fill: 456 tokens, 8 subagents → vocab **8009 → 8465**, 0 collisions, 0 missing corpus-wide. Backup `vocabulary.json.bak.pre-noun4-fill`. With this, all four content-word families are done (adverbs 98, verbs 104, adjectives 78, nouns 375); vocabulary.json grew 3922→8465 across the whole 2026-06-05 push.
- **Nouns Batch N3: +112 nouns / 5 new buckets (204 → 316 nouns; 300+ target met)** (2026-06-05, committed `c5ac5a6`). society-state (26), emotions-states (20), abstract-mind (15), abstract-structure (23), abstract-event (28); 10-mission case decks (abstract nouns lean on prep/gen where they don't pluralize). Regenerated `nouns/index.js` (14→19 groups). Vocab fill: 936 tokens, 14 subagents → vocab **7073 → 8009**, 0 collisions, 0 missing corpus-wide, 0 stragglers. Backup `vocabulary.json.bak.pre-noun3-fill`. Only N4 (3 merge-buckets, ~59 nouns) remains — see NEXT STEPS #2.
- **Nouns Batch N2: +76 nouns / 3 new buckets (128 → 204 nouns)** (2026-06-05, committed `549cac3`). people-roles (43), communication-media (17), education-science (16); 10-mission case decks (animate masc acc=gen handled). Regenerated `nouns/index.js` (11→14 groups). Vocab fill: 650 tokens, 10 subagents → vocab **6423 → 7073**, 0 collisions, 0 missing corpus-wide (0 stragglers this time — no session-limit interruption). Backup `vocabulary.json.bak.pre-noun2-fill`. 8 noun buckets (~170 nouns) remain — see NEXT STEPS #2.
