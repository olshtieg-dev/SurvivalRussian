...
**Content-word buildout COMPLETE + extended (2026-06-05/06): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 474 (375 core + 99 from the 1001–2000 tier).** All four families exceed target. `vocabulary.json` = 9424 entries, 0 missing tokens corpus-wide, 0 Cyrillic-in-natural. What remains is OPTIONAL: more of the 1001–2000 noun tier (~335 queued) — see below. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
- **Deeper noun coverage** — IN PROGRESS: N5 (2026-06-06) pulled the **1001–2000 frequency tier** (WebFetched from Wiktionary) and added the top **99 new nouns** into the existing 18 folders → nouns now **474**. **~335 more noun candidates from the 1001–2000 tier remain queued** (`/tmp/freq-new-nouns.txt`, lines 101+) for further N6… batches if owner wants. Going beyond 2000 would need an even deeper list.
...
- **~300+ most common nouns** — **DONE: 474** (19 buckets; N1–N4 built the top-1000 tier = 375; N5 added 99 from the 1001–2000 tier on 2026-06-06; ~335 more 1001–2000 candidates queued).
...
- Committed/pushed through the gloss-cleanup `76e8425`; **nouns Batch N5 about to be committed this session** (99 new nouns from the 1001–2000 tier into existing folders, `vocabulary.json` at **9424 entries**, 474 noun cards, 0 missing corpus-wide). Run `git log --oneline -3` for the latest hash. `.bak` backups intentionally untracked (11 now); a `*.bak` line in `.gitignore` would hide them all. — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
