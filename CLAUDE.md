...
Content-word buildout is mid-stream. Coverage now: **adverbs 98 ✅, verbs 104 ✅, adjectives 78 (39 pairs) ✅, nouns 128 (of ~340 target) — Batch N1 done, ~11 buckets remain.** Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
2. **Nouns — 40 → 300+ — Batch N1 DONE (128 nouns now); ~11 buckets remain.** Full plan: 336 new nouns across 18 semantic buckets, manifest at `/tmp/nouns-manifest.json` (ephemeral — regenerate via the noun-classifier agent prompt from `russian-frequency-1-1000.json` if gone, excluding the 40 covered). **Batch N1 (DONE, committed):** 7 new folders under `nouns/` — family (17), food-drink (9), nature-weather (25), animals (3), time-units (14), transport (4), work-money (16) = 88 nouns, 10 missions each, vocab filled, 0 missing. nouns/ now has 11 group folders, 128 cards.
   **REMAINING buckets for batches N2… (from the manifest):** people-roles (~43), communication-media (~17), education-science (~16), society-state (~26), emotions-states (~20), abstract-mind (~15), abstract-structure (~23), abstract-event (~28) → these are NEW folders. PLUS three MERGE-into-existing groups: body (~21 → merge into `body-person/`), places (~27 → merge into `motion-location/`), home-objects (~11 → merge into `household/`). For merges: add the new `.json` files into the existing folder and regenerate that folder's `index.js` PRESERVING its export names (bodyPersonNounsLessonSets, motionLocationNounsLessonSets, householdNounsLessonSets). Run each batch as: scaffold/manifest → parallel subagents write lessons (pass exact paths) → missing-token scan → parallel vocab fill → merge → validate 0 missing → commit.
...
- **~300+ most common nouns** — **currently 128** (original 4 buckets of 10 + Batch N1's 7 new buckets, 88 nouns, added 2026-06-05; ~11 buckets / ~210 nouns still to go — see NEXT STEPS #2).
...
- Committed/pushed through adjectives Batch 2 `e14fd96`; **nouns Batch N1 about to be committed this session** (88 new noun lessons, 7 new `nouns/` subfolders, `vocabulary.json` at **6423 entries**, 0 missing corpus-wide). Run `git log --oneline -3` for the latest hash. `.bak` backups remain intentionally untracked (now incl. `pre-noun1-fill`). — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
