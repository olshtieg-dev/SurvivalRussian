...
**Content-word buildout COMPLETE + extended (2026-06-05/06): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 571 (375 core + 196 from the 1001–2000 tier via N5+N6).** All four families exceed target. `vocabulary.json` = 10,197 entries, 0 missing tokens corpus-wide, 0 Cyrillic-in-natural. What remains is OPTIONAL: more of the 1001–2000 noun tier (~138 still uncovered) — see below. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
- **Deeper noun coverage** — IN PROGRESS: N5+N6 (2026-06-06) pulled the **1001–2000 frequency tier** (WebFetched from Wiktionary) and added **196 new nouns** into the existing 18 folders → nouns now **571**. **~138 noun candidates from the 1001–2000 tier remain** for N7+ batches if wanted; re-derive the still-uncovered list by ё-folded-excluding current `nouns/` labels from the 1001–2000 list (the proven recipe — see N5/N6 work-log). Going beyond rank 2000 would need an even deeper frequency source.
...
- **~300+ most common nouns** — **DONE: 571** (19 buckets; N1–N4 = top-1000 tier (375); N5+N6 = +196 from the 1001–2000 tier, 2026-06-06; ~138 of that tier still uncovered).
