...
**Content-word buildout COMPLETE (2026-06-05): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 375 ✅.** All four systematic content-word families hit/exceeded target. `vocabulary.json` = 8599 entries, 0 missing tokens corpus-wide. What remains is OPTIONAL polish, not core buildout — see below. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
- **~300+ most common nouns** — **DONE: 375** (19 buckets; built across N1–N4 on 2026-06-05; the last 3 buckets merged into existing body-person/motion-location/household folders).
...
- Committed/pushed through nouns Batch N4 `e60e767`; **standalone adjectives family about to be committed this session** (new `adjectives/` family, 17 cards, `vocabulary.json` at **8599 entries**, 0 missing corpus-wide). Content-word buildout fully complete (adverbs 98, verbs 104, adjectives 95, nouns 375). Run `git log --oneline -3` for the latest hash. `.bak` backups remain intentionally untracked (9 now); a `*.bak` line in `.gitignore` would hide them all. — 14 new `binary-adjectives/*.json`, regenerated `binary-adjectives/index.js`, `vocabulary.json` now **5747 entries**, 0 missing corpus-wide. Intentionally-untracked backups: `vocabulary.json.bak.{pre-expand-fill,pre-adverb-fill,pre-verb-fill,pre-adj-fill,pre-adj2-fill}` (a `*.bak` line in `.gitignore` would hide them). Next up: nouns (the big one) — see ">>> NEXT STEPS".
