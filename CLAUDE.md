...
**Content-word buildout COMPLETE + extended (2026-06-05/06): adverbs 98 ✅, verbs 104 ✅, adjectives 95 ✅, nouns 806 (375 core + 431 from the 1001–2000 tier via N5–N8).** All four families far exceed target; **the 1001–2000 frequency tier is now fully mined** (only non-noun residue left). `vocabulary.json` = 12,125 entries, 0 missing tokens corpus-wide, 0 Cyrillic-in-natural. Nothing required remains — see below for purely-optional ideas. Do these in order; each step is one orchestrated pass (generate lessons via parallel subagents → per-form vocab fill → validate 0 missing tokens → commit). The owner commits per batch — **commit only when asked.**
...
- **Deeper noun coverage** — the entire **1001–2000 frequency tier is now done** (N5–N8, 2026-06-06): 431 nouns added, nouns at **806**. N8 finished cleanly after the 5:10pm reset (88 partial files written before the limit + 51 generated after = 139). The ONLY way to add more common nouns now is a **deeper frequency source** (rank 2001+), which isn't in the repo — would need another WebFetch (e.g. Wiktionary 2001–3000). Treat noun coverage as effectively complete unless owner wants to go deeper.
...
- **~300+ most common nouns** — **DONE: 806** (19 buckets; N1–N4 = top-1000 tier (375); N5–N8 = +431 from the now-exhausted 1001–2000 tier, 2026-06-06). More would need a rank-2001+ frequency source.
