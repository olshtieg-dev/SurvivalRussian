...
// Next player after `fromID` (cyclic) that still holds cards, optionally
// skipping `skipID`. Falls back to the raw next player when everyone is empty.
function getNextActivePlayerID(G, ctx, fromID, skipID = null) {
  const order = ctx.playOrder || [];
  if (!order.length) return String(fromID);

  const start = order.indexOf(String(fromID));
  for (let step = 1; step <= order.length; step += 1) {
    const candidate = order[(start === -1 ? step - 1 : start + step) % order.length];
    if (skipID !== null && candidate === String(skipID)) continue;
    if ((G.hands[candidate] || []).length > 0) return candidate;
  }
  return getNextPlayerID(ctx, fromID);
}

// Assign the next bout's roles, advancing past empty-handed players once the
// deck is gone. `attackerSeed` is the intended attacker; `defenderAfter` is the
// seat we scan forward from to find the defender (skipping the attacker).
function setRoles(G, ctx, attackerSeed, defenderAfter) {
  let attacker = String(attackerSeed);
  if ((G.hands[attacker] || []).length === 0) {
    attacker = getNextActivePlayerID(G, ctx, attacker);
  }
  G.turnState.mode = 'attack';
  G.turnState.attackerID = attacker;
  G.turnState.defenderID = getNextActivePlayerID(G, ctx, defenderAfter ?? attacker, attacker);
}
...
function enumerateDurakMoves(G, ctx, playerID) {
  if (!G || !ctx || G.roundResult) return [];

  const seatID = String(playerID);
  const moves = [];

  if (G.turnState?.mode === 'attack' && seatID === String(G.turnState.attackerID)) {
    const hand = G.hands?.[seatID] || [];
    for (const card of hand) {
      moves.push({ move: 'attack', args: [card.id] });
    }
    // Pass only when there is nothing to attack with — keeps bots progressing.
    if (hand.length === 0) {
      moves.push({ move: 'pass', args: [] });
    }
  }

  if (G.turnState?.mode === 'defend' && seatID === String(G.turnState.defenderID)) {
    const hand = G.hands?.[seatID] || [];
    const pendingAttack = G.table?.find((pair) => pair.defendCard === null);
    if (pendingAttack) {
      for (const card of hand) {
        if (isCardBeatingAttack(card, pendingAttack.attackCard, G.trump)) {
          moves.push({ move: 'defend', args: [card.id] });
        }
      }
      moves.push({ move: 'take', args: [] });
    }
  }
...
const durakGame = {
  ...
  moves: {
    attack({ G, ctx, playerID, events }, cardId) {
      ...
      // Keep the assigned defender, but never point at the attacker or an
      // empty-handed seat.
      let defenderID = String(G.turnState.defenderID);
      if (defenderID === String(playerID) || (G.hands[defenderID] || []).length === 0) {
        defenderID = getNextActivePlayerID(G, ctx, playerID, String(playerID));
      }
      G.turnState.defenderID = defenderID;
    ...
    defend({ G, ctx, playerID, events }, cardId) {
      ...
      // Successful defence: the defender wins the bout and becomes the attacker.
      refillHands(G, ctx, String(playerID));
      setRoles(G, ctx, String(playerID), String(playerID));
    ...
    pass({ G, ctx, playerID, events }) {
      ...
      // Attacker has nothing to play (empty hand): hand off to the next active
      // player. Voluntary passing while holding cards is not allowed (enumerate
      // only offers pass with an empty hand), which guarantees forward progress.
      refillHands(G, ctx, String(playerID));
      const nextAttacker = getNextActivePlayerID(G, ctx, playerID);
      setRoles(G, ctx, nextAttacker, nextAttacker);
    ...
    take({ G, ctx, playerID, events }) {
      ...
      G.table = [];
      // Defender failed and picks up the cards, so they are skipped. The same
      // attacker keeps the initiative (and keeps shedding cards once the deck is
      // empty), which is what guarantees the round terminates.
      const taker = String(playerID);
      const currentAttacker = String(G.turnState.attackerID);
      refillHands(G, ctx, currentAttacker);
      setRoles(G, ctx, currentAttacker, taker);
