...
## ▶ NEXT SESSION — TODO (picked up here)

1. **Quiz bank — continue per-set granularity:** generate quizzes for **lexical-sets
   (~35 sets) and conversation, ONE TEST PER SET** (not per family). This is the batch
   that pushes the *test count* toward the low hundreds (~50–70 more tests). Then go from
   there: pro-forms, cases-nouns, spatial-motion, the curated main sets (essentials,
   grocery, kitchen, household, slang), and the missing **conjunctions** quiz for the
   mislabeled `comparisons/` folder.
   - Current bank: **41 quizzes / 820 questions** (grammar families + content-word
     families + all 19 noun groups). Pipeline is proven — same subagent recipe, regen
     `src/data/quizzes/index.js` from the directory, validate, live-build check.
2. Nothing is committed — owner runs their own git workflow.
...
## Workstream 5 — Knowledge tests & review: 🟡 v1 WIRED 2026-06-13

**Built this session** (optional, encouraged-not-mandatory):
- **Quiz button** in the RightRail (amber clipboard icon) → **`QuizOverlay`**: list of tests
  (with a "Studied" tag for chunks the learner has worked + past best score), take a
  20-question multiple-choice test, get scored, then **review** every question with the
  correct answer highlighted and an explanation. Retake any time.
- **Results** persisted client-side (`useQuizResults`, `survival-russian-quizzes-v1`):
  attempts, best/last score per chunk.
- **Question bank** (`src/data/quizzes/`, registry `index.js`): one quiz per curriculum
  chunk (chunkId = lesson family id), **20 questions each, grounded in the actual lesson
  sentences/vocab/grammar** (generated by subagents that read the real lesson JSON). Easy-
  to-moderate — answerable if you actually read the lessons. "Not every sentence has a
  question" — each test samples its chunk.
- **First batch (8 quizzes / 160 questions)**: participles, gerunds, conditional,
  motion-verbs, diminutives, numerals, possessives, passive-voice. Schema-validated
  (4 options, 0-based answer, balanced answer keys); live build clean.
- **Batch 2 (2026-06-13): +14 quizzes / 280 Q → 22 quizzes / 440 Q total.** Added the
  content-word families at group level — verbs (1st/2nd/mixed/irregular conjugation),
  adverbs (time/degree/manner/place/predicative/quantity), adjectives (descriptive/colors),
  binary-adjectives (antonym pairs), and a comparatives/superlatives quiz. Expanded the
  question palette to the "YouTube vibe": sentence completion, EN→RU production, RU→EN
  comprehension, grammatical-usage (case/aspect/agreement/conjugation), antonyms,
  odd-one-out. All schema-validated; live build clean.
  - NOTE: the `comparisons/` lesson folder is actually **conjunctions** (а, что, чтобы,
    если, и, или, но…), not comparatives — so the comparatives quiz uses chunkId
    `comparatives` (decoupled). **TODO: a conjunctions quiz for the real `comparisons`
    folder**, and per-folder-name cleanup is worth considering.
- **Batch 3 (2026-06-13): +19 quizzes / 380 Q → 41 quizzes / 820 Q total.** All 19 noun
  groups (family, food-drink, animals, time-units, transport, work-money, education-science,
  nature-weather, communication-media, society-state, emotions-states, body-person,
  people-roles, household, motion-location, abstract, abstract-mind, abstract-structure,
  abstract-event). Noun-focused styles: vocab (RU↔EN), case identification, case-form
  selection, gender, plural (incl. irregulars глаза/уши/друзья/времена), animacy, location
  idioms. Agents SAMPLED the big groups (100+ cards) rather than reading all. Registry
  regenerated from the directory (41 imports); live build clean.
- **Scaling — remaining to reach a few hundred TESTS** (we're at 41 tests / 820 Q):
  pro-forms, cases-nouns, spatial-motion (group/family level); **lexical-sets (~35 sets)
  and conversation — best done per-SET → ~50–70 more tests**; the curated main sets
  (essentials, grocery, kitchen, household, slang); and the missing **conjunctions** quiz
  for the mislabeled `comparisons/` folder. Per-set granularity on lexical-sets/conversation
  is what pushes the test COUNT into the low hundreds.
